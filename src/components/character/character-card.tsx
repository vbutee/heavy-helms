  ...
  return (
    ...
    >
      <div className="bg-gradient-to-b from-amber-800 to-amber-950 text-white">
        ...
        {/* Character Info */}
        <div className="p-4">
          <h3 className={`${bokor.className} text-2xl mb-2 text-amber-300`}>{name}</h3>
          
          <div className="flex justify-between mb-4">
            <div>
              <span className={`inline-block px-3 py-1 rounded-full bg-amber-950 ${stanceStyles[stance]}`}>
                {stance.charAt(0).toUpperCase() + stance.slice(1)}
              </span>
            </div>
            <div className="flex space-x-2">
              <span title={weapon} className="inline-block px-2 py-1 bg-amber-900 rounded-md">
                {getWeaponIcon(weapon)} {weapon}
              </span>
              <span title={armor} className="inline-block px-2 py-1 bg-amber-900 rounded-md">
